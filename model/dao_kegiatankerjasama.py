...
def getPrestasiRekognisiDosen():
    ...
    group by tahun, judul_rekognisi, wilayah
    order by tahun desc
    ''')
...
def getRerataJumlPesertaKulUm():
    return pd.read_sql('''
    select tahun, avg(jumlah) 'Rata-rata Peserta' from
    (select tahun, count(id_mahasiswa) jumlah from fact_kegiatan_mahasiswa fkm
    ...
    group by tahun,dk.id_kegiatan) kegiatan
    group by tahun
    order by tahun desc
    ''',con)
