...
def getJabfungperTahun():
    return pd.read_sql('''
    select jabatan.tahun Tahun,jabatan.Jumlah 'Jumlah Pejabat', dosen.Jumlah 'Total Dosen',(jabatan.Jumlah/dosen.Jumlah)*100 '%' from
...
def getPersenJabfungperTahun():
    ...
    select jabatan.tahun                         Tahun,
       ...
       (jabatan.Jumlah / dosen.Jumlah) * 100 '%'
    ...
    where jabatan.tahun = dosen.tahun
    group by Tahun, Jabatan, `Jumlah Jabatan`, `Jumlah Dosen`, `%`
    order by Tahun desc;
    ''',con)
